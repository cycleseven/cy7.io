...
Tests could definitely do a lot more for us in future. The current approach is a tradeoff based on the nature of the project: there's very little in the way of custom runtime behaviour on [cy7.io](http://cy7.io) so far. And many categories of problems that are worth worrying about can be detected through type checking, linting, automated accessibility checking, or simply by running `gatsby build`.
